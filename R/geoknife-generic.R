...
#'@param show_progress logical (optional) display progress bar?
...
#'}
#'@export
geoknife <- function(stencil, fabric, knife = webprocess(...), show_progress = TRUE, ...){
  ...
  if (knife@wait){
    wait(geojob, sleep.time = knife@sleep.time, show_progress = show_progress)
  }
